...
if (isset($_GET['id_buku'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (mysqli_query($conn, $sql_update)) {
            echo "<script>alert('Buku berhasil diperbarui!'); window.location.href='data_buku.php';</script>";
        } else {
...
<div class="container">
    ...
    <form action="edit_buku.php?id_buku=<?php echo $id_buku; ?>" method="post" enctype="multipart/form-data">
        ...
        <input type="number" id="jumlah_halaman" name="jumlah_halaman" value="<?php echo htmlspecialchars($buku['jumlah_halaman']); ?>" required>

        <label for="jumlah_copy">Jumlah Copy</label>
        <input type="number" id="jumlah_copy" name="jumlah_copy" value="<?php echo htmlspecialchars($buku['jumlah_copy']); ?>" required>

        <label for="tersedia">Tersedia</label>
        <input type="number" id="tersedia" name="tersedia" value="<?php echo htmlspecialchars($buku['tersedia']); ?>" required>

        <label for="deskripsi_buku">Deskripsi Buku</label>
        <textarea id="deskripsi_buku" name="deskripsi_buku" rows="4" required><?php echo htmlspecialchars($buku['deskripsi_buku']); ?></textarea>

        <div class="current-cover">
            <p>Cover Saat Ini:</p>
            <img src="uploads/<?php echo htmlspecialchars($buku['cover']); ?>" alt="Cover Buku">
        </div>

        <label for="cover">Ganti Cover Baru (Opsional)</label>
        <input type="file" id="cover" name="cover">

        <button type="button" onclick="window.location.href='data_buku.php'" class="btn-back">Back</button>
