...
if (isset($_GET['id_buku'])) {
    ...
    if ($data) {
        ...
        if (!empty($data['cover']) && file_exists('uploads_digital/' . $data['cover'])) {
            unlink('uploads_digital/' . $data['cover']);
        }
        ...
        if ($query_delete) {
            echo "<script>alert('Buku berhasil dihapus!'); window.location.href='data_bukudigital.php';</script>";
        } else {
