...
if ($keyword) {
    ...
    $sql = "SELECT * FROM buku WHERE jenis = 'cetak' ORDER BY judul ASC";
}
...
footer {
    ...
    justify-content: center;
    gap: 20px;
    background-color: rgba(51, 51, 51, 0.8); /* Transparansi pada footer */
    padding: 20px 0;
}
...
<section class="collection">
    <h2>KOLEKSI FISIK DEXENRY</h2>
    <form method="GET" class="search-form">
    <input type="text" name="keyword" placeholder="Cari judul, penulis, penerbit, atau tahun terbit..." value="<?php echo isset($_GET['keyword']) ? htmlspecialchars($_GET['keyword']) : ''; ?>">
    <button type="submit" name="cari"><i class="bi bi-search"></i> Cari</button>
    </form>

    <a href="tambah_buku.php" class="add-book-btn">Tambah Buku Baru</a>
        ...
        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
            <div class="book-item <?php echo $row['jenis']; ?>">
                <a href="#">
                    ...
                    <span class="jenis <?php echo $row['jenis']; ?>">
                        <?php echo ucfirst($row['jenis']); ?>
                    </span>
                </a>
                <!-- Admin Controls -->
                <div class="admin-controls">
                    <a href="edit_buku.php?id_buku=<?php echo $row['id_buku']; ?>" title="Edit Buku"><i class="bi bi-pencil-square"></i></a>
                    <a href="hapus_buku.php?id_buku=<?php echo $row['id_buku']; ?>" title="Hapus Buku" onclick="return confirm('Apakah Anda yakin ingin menghapus buku ini?')"><i class="bi bi-trash"></i></a>
                </div>
            </div>
        <?php endwhile; ?>
    </div>
